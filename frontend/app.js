 ...
 * Seller Hub Dashboard, Bug-Free Checkout Form, and Admin CMS Controller.
 */
...
async function handleLoginSubmit(event) {
  ...
  try {
    ...
    if (data.success) {
      ...
      // Auto-open Seller Hub if user is a Seller
...
async function handleRegisterSubmit(event) {
  ...
  try {
    ...
    if (data.success) {
      ...
      // Auto-open Seller Hub on Seller Registration
      if (role === 'seller' || role === 'admin') {
        setTimeout(() => {
          openModal('sellerModal');
        }, 400);
    ...
    state.user = { id: 'usr_' + Date.now(), name, email, role, storeName };
    state.token = 'demo_token_123';
    localStorage.setItem('fvr_user', JSON.stringify(state.user));
    localStorage.setItem('fvr_token', state.token);
    ...
    updateUserUI();
    closeModal('authModal');
    showToast(`Registered as ${role.toUpperCase()}!`, 'success');
...
/* ==========================================================================
   SELLER HUB SUB-TABS & CATALOG MANAGEMENT
   ========================================================================== */
...
function renderSellerProductsTable() {
  ...
  tbody.innerHTML = state.products.map(p => `
    <tr>
      <td><img src="${p.images[0]}" style="width:45px; height:45px; border-radius:6px; object-fit:cover;"></td>
      ...
      <td>
        <button class="btn btn-secondary" onclick="deleteProduct('${p._id}')" style="padding:4px 8px; color:var(--danger);"><i class="fa-solid fa-trash"></i> Remove</button>
      </td>
    </tr>
  `).join('');
}
...
function openModal(modalId) {
  ...
  // Load seller catalog table if sellerModal opened
